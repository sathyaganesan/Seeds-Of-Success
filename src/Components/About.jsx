...
export default function About() {
  ...
  const scrollToourStory = useCallback(() => {
    ourStory.current?.scrollIntoView({ behavior: 'smooth', block: "center" });
  }, []);

  const scrollToourApproach = useCallback(() => {
    ourApproach.current?.scrollIntoView({ behavior: 'smooth', block: "center" });
  }, []);

  const scrollToourMission = useCallback(() => {
    ourMission.current?.scrollIntoView({ behavior: 'smooth', block: "center" });
  }, []);

    const scrollToourImpact = useCallback(() => {
    ourImpact.current?.scrollIntoView({ behavior: 'smooth', block: "center" });
  }, []);
  ...
  <>
    <div >
      ...
      <div ref={ourStory} className='page-content'>
        <h2>Our Story</h2>
      ...
      <div  ref={ourApproach} className='page-content'>
        <h2>Our Approach</h2>
        <p>Seeds of Success is a 1-on-1 tutoring program that is structured yet flexible to ensure both access and quality.  Our approach includes:</p>
        <ul>
      ...
      <div ref={ourMission} className='page-content'>
        <h2>Our Mission</h2>
        <p>Our mission is to equip rural Tamil Nadu children with practical English skills through supportive 1-on-1 sessions with Tamil-origin tutors abroad, 
          building confidence and nurturing pride in cultural roots for both tutors and students.
        </p>
      </div>
      <div ref={ourImpact} className='page-content'> 
        <h2>Our Impact</h2>
        <p>Every tutoring session is a small step toward a bigger change — a child who can speak with confidence, a family uplifted from cycles of poverty,
